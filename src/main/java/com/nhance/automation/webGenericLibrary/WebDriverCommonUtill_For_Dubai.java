 ...
 *
 */
public class WebDriverCommonUtill_For_Dubai extends WebDriverIgnit {
	...
	public void implicitWait() {
		getDubaiDriver().manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);

	}
	
	// Method for wait using Explicit wait condition till element present in UI
	public void explicitlyWait(WebElement element) {
	  WebDriverWait wait = new WebDriverWait(getDubaiDriver(), 60);
	...
	Wait<WebDriver> wait = new FluentWait<WebDriver>(getDubaiDriver()).withTimeout(Duration.ofSeconds(100))
	...
	public void maximize() {
		getDubaiDriver().manage().window().maximize();
	}

	// Method for maximize the Browser window
	public void refreshPage() {
		getDubaiDriver().navigate().refresh();
	}

	// Method for maximize the Browser window
	public void backPage() {
		getDubaiDriver().navigate().back();
	}

	// Method for maximize the Browser window
	public void forward() {
		getDubaiDriver().navigate().forward();
	}
	...
	public void deleteAllCookies() {
		getDubaiDriver().manage().deleteAllCookies();
	}
	
	// Method for Accepting alert
	public void acceptAlert() {
		Alert alt = getDubaiDriver().switchTo().alert();
		alt.accept();
	}
	...
	public void frameHandling(int index) {
		...
		try {
			getDubaiDriver().switchTo().frame(index);
	...
	public void frameHandling(WebElement wb) {
		...
		try {
			getDubaiDriver().switchTo().frame(wb);
		} catch (NoSuchFrameException e) {
			System.out.println(e.getMessage());
		}
	...
	public void frameHandling(String FrameName) {
		...
		try {
			getDubaiDriver().switchTo().frame(FrameName);
		} catch (NoSuchFrameException e) {
			System.out.println(e.getMessage());
		}
	...
	public void mouseOver(WebElement wb) {
		Actions act = new Actions(getDubaiDriver());
		fluentWait(wb);
	...
	{
	Actions actions = new Actions(getDubaiDriver());
	...
	public void getscreenshot(String destPath) throws IOException {
		EventFiringWebDriver efwd = new EventFiringWebDriver(getDubaiDriver());
	...
	public void scrollingUpUsingScriptExecutor() throws InterruptedException {
		JavascriptExecutor jse = (JavascriptExecutor) getDubaiDriver();
		for (int i = 0; i < 1; i++) {
	...
	{
		JavascriptExecutor js = (JavascriptExecutor)getDubaiDriver();
	...
	// Capture the attribute value
	 ...
	 public  void uploadFile(String fileLocation) {
		 ...
		 WebElement fileInput = getDubaiDriver().findElement(By.xpath("//input[@type='file']"));
